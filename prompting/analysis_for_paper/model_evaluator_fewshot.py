...
class ModelEvaluator:
    ...
    def calculate_strict_consistency(self, figurative_filepath, literal_filepath, model_and_id):
        ...
        combined_correctness.to_csv(f"robustness_correct_idioms_fewshot/{model_and_id}.csv", index=False)
...
def print_scores_out(dictionary):
    for key,value in dictionary.items():
        print(f"{key}:\t{value}")

evaluator = ModelEvaluator(predictions_dir="../../xshot_prompting_samePrompt/cleaned_model_outputs")
...
print_scores_out(fig_f1_averages)

print(f"\nlit_f1_model_results: \t{lit_f1_model_results}")
print(f"\nlit_f1_averages:\t{lit_f1_averages}")
print_scores_out(lit_f1_averages)



lc_results = evaluator.get_loose_consistency()

fig_lc_model_results, fig_lc_averages = evaluator.average_scores(lc_results, "figurative", 1)
lit_lc_model_results, lit_lc_averages = evaluator.average_scores(lc_results, "literal", 1)

print(f"\nfig_lc_model_results:\t{fig_lc_model_results}")
print(f"\nfig_lc_averages:\t{fig_lc_averages}")
print_scores_out(fig_lc_averages)

print(f"\nlit_lc_model_results: \t{lit_lc_model_results}")
print(f"\nlit_lc_averages:\t{lit_lc_averages}")
print_scores_out(lit_lc_averages)

sc_results = evaluator.get_strict_consistency()
print(f"\nsc_results:\t{sc_results}")

sc_averages = evaluator.sc_average_scores(1)
print(f"sc_averages:\t{sc_averages}")
print_scores_out(sc_averages)
